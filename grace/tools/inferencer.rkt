...
(define inferencer%
  (class object%
    ...
    (define current-entry #f); (list start end var-name type-name inf-type)
    (define offset (send definitions-text line-end-position 0 #f))
    (define current-highlight-key #f)
    
    ;; Do the actual inference
    (define inference-list 
          (infer-types (parse-string
                        (open-input-string 
    ...
    (define/public (insert-type)
      (match-define (list start end var-name type-name inf-type) current-entry)
    ...
    (define/public (process-next)
      ...
      (if (empty? inference-list)
          ...
          (begin
            ...
            (match-let ([(list start end var-name type-name inf-type) current-entry])
